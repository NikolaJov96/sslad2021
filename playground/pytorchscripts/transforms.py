...
class Compose(object):
    ...
    def __call__(self, image, target):
        for t in self.transforms:
            # print('t', t)
            # print(target)
            # print('boxes', type(target['boxes']))
            # print('labels', type(target['labels']))
            # print('masks', type(target['masks']))
            # exit(1)
            # image = t(image)
            # target = t(target)
            image, target = t.forward(image, target)
        return image, target


class RandomHorizontalFlip2(T.RandomHorizontalFlip):
